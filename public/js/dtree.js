...
dTree.prototype.node = function(node, nodeId) {
	...
	else if (node.pid == 0 || (node._hc && !node._io))
	{
		str += 'dTreeRoot';
	}
	else
	...
	if (node.pid == 0 || (node._hc && !node._io)){
		str += node.name;
	}else{
		str += "&nbsp;&nbsp;&nbsp;"+node.name;
	}
    ...
    if (this.root.id != node.pid) {
        ...
        if (node._hc) {
			...
			if(this.config.useLines){
				str += '<img align="absmiddle"  src="' + ( (this.config.useLines) ? ((node._ls) ? this.icon.joinBottom : this.icon.join ) : this.icon.empty) + '" alt="" />';
			}
		}
    }
    return str;
