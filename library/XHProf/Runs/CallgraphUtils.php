...
class CallgraphUtils {
    ...
    private static $xhprof_legal_image_types = [
        "jpg" => 1,
        "gif" => 1,
        "png" => 1,
     ...
     * @author cjiang
     */
    public static function xhprof_generate_image_by_dot($dot_script,$type,$cmd='',$img_path='') {
        ...
        $process = proc_open($cmd, $descriptorspec, $pipes, $file, array());
     ...
     * @author cjiang
     */
    public static function xhprof_generate_dot_script($raw_data, $threshold, $source, $page,
            ...
            while ($node) {
                ...
                if (isset($children_table[$node])) {
                    ...
                    foreach ($children_table[$node] as $child) {
                        ...
                        if ($max_child === null ||
                            abs($raw_data[XHProfLib::xhprof_build_parent_child_key($node,
                                $child)]["wt"]) >
                                abs($raw_data[XHProfLib::xhprof_build_parent_child_key($node,
                                    $max_child)]["wt"])) {
                                    $max_child = $child;
    ...
    public static function  xhprof_render_diff_image($xhprof_runs_impl, $run1, $run2,
        ...
        $script = self::xhprof_generate_dot_script($run_delta, $threshold, $source,
            null, null, true,
     ...
     * @author cjiang
     */
    public static function xhprof_get_content_by_run($xhprof_runs_impl, $run_id, $type,
        ...
        $script = self::xhprof_generate_dot_script($raw_data, $threshold, $source,
     ...
     * @author cjiang
     */
    public static function xhprof_render_image($xhprof_runs_impl, $run_id, $type, $threshold,
        $func, $source, $critical_path) {
        $content = self::xhprof_get_content_by_run($xhprof_runs_impl, $run_id, $type,
            $threshold,
            $func, $source, $critical_path);
        if (!$content) {
