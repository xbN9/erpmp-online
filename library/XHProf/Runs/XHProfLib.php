...
class XHProfLib {
    ...
    private static $pc_stats;
    private static $metrics;
    private static $diff_mode;
    private static $sortable_columns;
    private static $sort_col;
    private static $display_calls;

    const XHPROF_STRING_PARAM='';
    ...
    public static function init_metrics($xhprof_data, $rep_symbol, $sort, $diff_report = false) {
        ...
        if (!isset($xhprof_data["main()"]["wt"])) {
            ...
            self::$display_calls = false;
        } else {
            self::$display_calls = true;
        }
        // parent/child report doesn't support exclusive times yet.
        // So, change sort hyperlinks to closest fit.
        if (!empty($rep_symbol)) {
            self::$sort_col = str_replace("excl_", "", self::$sort_col);
        }
        if (self::$display_calls) {
            self::$stats = array("fn", "ct", "Calls%");
        } else {
            self::$stats = array("fn");
        }
        self::$pc_stats = self::$stats;
        ...
        foreach ($possible_metrics as $metric => $desc) {
            if (isset($xhprof_data["main()"][$metric])) {
                ...
                self::$stats[] = $metric;
                self::$stats[] = "I" . $desc[0] . "%";
                self::$stats[] = "excl_" . $metric;
                self::$stats[] = "E" . $desc[0] . "%";
                // parent/child report for a function: we can
                // only breakdown inclusive times correctly.
                self::$pc_stats[] = $metric;
                self::$pc_stats[] = "I" . $desc[0] . "%";
            }
        }
        return ['metrics'=>self::$metrics,'stats'=>self::$stats,'pc_stats'=>self::$pc_stats];
    }
    ...
    public static function xhprof_normalize_metrics($raw_data, $num_runs) {
        ...
        if (isset($raw_data["==>main()"]) && isset($raw_data["main()"])) {
            self::xhprof_error("XHProf Error: both ==>main() and main() set in raw data...");
        }
        foreach ($raw_data as $parent_child => $info) {
     ...
     */
    public static function xhprof_aggregate_runs($xhprof_runs_impl, $runs,
        $wts, $source="phprof",
        ...
        foreach ($runs as $idx => $run_id) {
            ...
            if ($use_script_name) {
                ...
                if ($page) {
                    ...
                    $raw_data[self::xhprof_build_parent_child_key("main()",
                        "__script::$page")]
                        = $fake_edge;
                } else {
                    $use_script_name = false;
                ...
                if ($use_script_name) {
                    ...
                    if (substr($parent_child, 0, 9) == "main()==>") {
                        ...
                        if (substr($child, 0, 10) != "__script::") {
                            $parent_child = self::xhprof_build_parent_child_key("__script::$page",
                                $child);
                        }
                    }
                }
        ...
        $data['raw'] = self::xhprof_normalize_metrics($raw_data_total,
            $normalization_count);
     ...
     *                       used.
     */
    public static function xhprof_param_init($params) {
        /* Create variables specified in $params keys, init defaults */
        foreach ($params as $k => $v) {
            switch ($v[0]) {
            case 'xhprof_string_param':
                $p = self::xhprof_get_string_param($k, $v[1]);
                break;
            case 'xhprof_uint_param':
                $p = self::xhprof_get_uint_param($k, $v[1]);
                break;
            case 'xhprof_float_param':
                $p = self::xhprof_get_float_param($k, $v[1]);
                break;
            case 'xhprof_bool_param':
                $p = self::xhprof_get_bool_param($k, $v[1]);
                break;
            default:
                self::xhprof_error("Invalid param type passed to xhprof_param_init: "
                    . $v[0]);
                exit();
            }
            if ($k === 'run') {
